...
@section("content")
    ...
    <div class="my-3 pb-3 p-1 bg-white rounded shadow-sm">
        <div class="col-md-12 order-md-1">
            ...
            <form id="form_element" action="{{ route('management.user.update', $nameuser['id']) }}" class="needs-validation" novalidate="" method="POST">
                @csrf
                @method("PUT")
                    ...
                    <div class="mb-12 col-md-5 col-12">
                        ...
                        <select class="custom-select d-block w-100" id="nivel" name="nivel" required="">
                            <option value="">Selecione...</option>
                                <option value="2" {{ ($nameuser["type"] == '2' ? 'selected' : '')  }}>Desenvolvedor</option>
                                <option value="0" {{ ($nameuser["type"] == '0' ? 'selected' : '')  }}>Administrador</option>
                                <option value="1" {{ ($nameuser["type"] == '1' ? 'selected' : '')  }}>Vendedor</option>
                        </select>
                    </div>
                </div>
                <hr class="mb-4">
                <button class="btn btn-primary btn-lg btn-block" id="save_submit" type="submit">Atualizar</button>
            </form>
        </div>
    </div>
        ...
        $(document).ready(function () {
            $("#save_submit").click(function (e) {
                ...
                if(password.value.length <= 0) {
                    $("#password").notify("Para atualizar o cadastro, entre com a senha novamente!", "error");
                }
